...
/**
 * Reset the database — the ONLY reliable way to empty a sql.js database.
 *
...
export function nukeDb(): void {
  ...
  try {
    ...
    oldDb.run('DELETE FROM processing_state');
    oldDb.run('DELETE FROM transaction_log');
...
export async function initDb(): Promise<Database> {
  ...
  try {
    ...
    `;
    _db.run(transactionLogInit);

    console.log(`SQLite (sql.js) initialisé — ${dbPath}`);
  } catch (err) {
...
/**
 * Directly execute SQL on the raw sql.js DB and flush to disk.
 * Use ONLY for admin operations (e.g. clearing all data) where the normal wrapper
 * would add overhead or risk partial saves.
 */
