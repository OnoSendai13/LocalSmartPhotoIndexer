...
try {
    ...
    $reportPath = Join-Path $projectRoot 'migration-report.txt'
    $migrationScript = Join-Path $projectRoot 'migrate-thumbnails.js'
    ...
    # 2) Vérification dossier server
    if (-not (Test-Path $serverPath)) {
        throw "Dossier server introuvable: $serverPath"
    }

    if (-not (Test-Path $migrationScript)) {
        throw "Script migrate-thumbnails.js introuvable: $migrationScript"
    }
    ...
    Write-Info "Lancement de la migration..."
    ...
    if ($exitCode -eq 0) {
        Write-Ok "Migration terminée avec succès."
    }
